---
title: "SPY American Option Pricing Model"
header-includes:
  - \usepackage{setspace}\doublespacing
subtitle: |
  | FE 620 Final Project
  |
  | https://github.com/tej100/American_Opt_Pricing
  |
...
The following R libraries were utilized throughout our analysis.
...
On December 1st, 2023, the SPDR S&P 500 ETF (SPY) American options chain data was retrieved from the Bloomberg Terminal as an Excel file. The dataset included a total of 160 instances, evenly split between 80 calls and 80 puts. This information encompassed options with 20 different strikes, ranging from \$450 to \$469, with intervals of $1. The options had four distinct expiration dates, corresponding to 2-week, 4-week, 6-week, and 8-week time-to-maturities. The factors available in the dataset were strike price, ticker (which included the contract's expiration date), National Best Bid Offer (NBBO), last price, Bloomberg's market implied volatility, and trade volume.
...
To ensure data integrity, a comprehensive data cleaning process was conducted, systematically removing any non-numeric or missing values. This meticulous approach aimed to create a dataset devoid of common data input errors and omissions, establishing a solid foundation for further analysis and interpretation. The following bullets some of the preprocessing techniques we implemented.

- **Reformatted** corrupt **column headers** for convenient data frame access.
- **Removed null rows** in the data set as a result of unnecessary sub headers within the data.
...
- **Filtered** the imported options data by **calls and puts**. This led to the rest of our cleaning to be implemented on each of our call and put dataframes.
...
- Computed the **mid-price**, which is the midpoint between the best bid and best ask, used as the most common reference for the fair market price.
- **Standardized the units** of all input variables for modeling purposes
- Removed excess columns not needed for pricing
...
- Stripped the option expiration date from the contract ticker and performed datetime operations to calculate our time-to-maturity, $\tau$.
...
- Adding a column to keep track of $dt$ for hedging purposes (only demonstrated on call options).
...
Below, we've illustrated our binomial asset pricing model.
...
output<-bind_rows(apply(calls, 1, function(row)
...
delta_hedge<-american_option_price(
    S0 = S$SPY.Adjusted[1],
    r = rf,
    sigma = tau1_K458_puts$IVM,
    tau = tau1_K458_puts$tau-(1/252),
    K = tau1_K458_puts$Strike,
    steps = N,
    opt_type = "put",
    greeks = TRUE,
    # data was from 12/01/23, next ex_div data is 12/16/23 -> 16-1=15 days
    div_sched = div_sched_calc(tau1_K458_puts$tau, div_freq, div_amt, 11/252)
  )
...
for(i in c(2:length(S$SPY.Adjusted))){
  delta_hedge<-rbind(delta_hedge,american_option_price(
    ...
    r = rf,
    sigma = tau1_K458_puts$IVM,
    tau = tau1_K458_puts$tau-(i/252),
    K = tau1_K458_puts$Strike,
    steps = N,
    opt_type = "put",
    greeks = TRUE,
    # data was from 12/01/23, next ex_div data is 12/16/23 -> 16-1=15 days
    div_sched = div_sched_calc(tau1_K458_puts$tau-(i/252), div_freq, div_amt, 11/252)
  ))
