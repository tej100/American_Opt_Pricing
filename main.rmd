...
The same trend can be seen when pricing puts, though to less of a degree as tau increases. This increase in error can likely be attributed to our use of constant interest rates and volatility, which are exacerbated when there is more time for the model to account for. 

## Evaluating the Greeks

### Delta
```{r}
plot(calls$Strike[1:20],calls$Delta[1:20], main = "Delta vs Strike")
points(calls$Strike[21:40],calls$Delta[21:40])
points(calls$Strike[41:60],calls$Delta[41:60])
points(calls$Strike[61:80],calls$Delta[61:80])
```
...
### Gamma
```{r}
plot(calls$Strike[1:20],calls$Gamma[1:20],ylim=c(0,.04), main = "Gamma vs Strike")
points(calls$Strike[21:40],calls$Gamma[21:40])
points(calls$Strike[41:60],calls$Gamma[41:60])
points(calls$Strike[61:80],calls$Gamma[61:80])
```

### Theta
```{r}
plot(calls$Strike[1:20],calls$Theta[1:20],ylim = c(-62,-30), main = "Theta vs Strike")
points(calls$Strike[21:40],calls$Theta[21:40])
points(calls$Strike[41:60],calls$Theta[41:60])
points(calls$Strike[61:80],calls$Theta[61:80])
```


### Vega
```{r}
plot(calls$Strike[1:20],calls$vega[1:20],ylim=c(30,95), main = "Vega vs Strike")
points(calls$Strike[21:40],calls$vega[21:40])
points(calls$Strike[41:60],calls$vega[41:60])
points(calls$Strike[61:80],calls$vega[61:80])
```


### Rho
```{r}
plot(calls$Strike[1:20],calls$rho[1:20],ylim=c(0,80), main = "Rho vs Strike")
points(calls$Strike[21:40],calls$rho[21:40])
points(calls$Strike[41:60],calls$rho[41:60])
points(calls$Strike[61:80],calls$rho[61:80])
```
